...
<section
  class="flex max-h-80 shrink-0 flex-col border-t border-[var(--color-border)] bg-[var(--color-surface-2)]"
...
>
  ...
  <div class="flex items-center gap-3 px-5 py-2">
    ...
    <div class="relative ml-auto">
      <button
        class="rounded-lg bg-[var(--color-accent)]/20 px-3 py-1 text-xs text-[var(--color-accent)] transition hover:brightness-125 disabled:opacity-40"
      ...
      {#if addMenuOpen}
        ...
        ></button>
        <div
          class="absolute right-0 z-40 mt-1 flex w-32 flex-col rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-1 shadow-lg"
          role="menu"
        >
          {#each KINDS as { kind, label } (kind)}
            <button
              class="rounded px-2 py-1 text-left text-xs transition hover:bg-[var(--color-surface-2)]"
              role="menuitem"
              onclick={() => add(kind)}
            >
              {label}
  ...
  <div class="flex flex-1 items-start gap-2 overflow-x-auto px-5 pb-3">
    ...
    {:else if effects.length === 0}
      <p class="py-4 text-xs text-[var(--color-muted)]">
        No effects yet. Use <span class="text-[var(--color-accent)]">+ FX</span> to add one.
      </p>
    {:else}
